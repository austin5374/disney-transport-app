...
const FEATURES: { icon: keyof typeof Ionicons.glyphMap; title: string; body: string }[] = [
  ...
  },
  {
    ...
    title: 'Transit Map',
    body: 'A pannable schematic of the monorail, Skyliner, and watercraft network, with live departures on the lines themselves. Select any line to highlight it and read its status.',
  },
  {
    icon: 'bookmark-outline',
    title: 'Saved Trips',
    body: 'Keep a trip you take often. A saved trip stores the question, not the answer, so opening it re-checks against the day\'s service.',
  },
];

type Props = { navigation: StackNavigationProp<MoreStackParamList, 'About'> };

export default function AboutScreen({ navigation }: Props) {
  return (
    <View style={styles.screen}>
      <AppHeader showBack onBack={() => navigation.goBack()} title="About" subtitle={Brand.title} />

      <ScrollView contentContainerStyle={styles.scroll}>
        <Section header="What's Inside" flush>
          {FEATURES.map((f, i) => (
            <View key={f.title}>
              {i > 0 && <Divider />}
        ...
        <Section header="About This App">
          ...
          <Text style={styles.body}>
            Because all of that computes on your device, the app keeps working with no
            signal at all. Add it to your home screen and it runs offline.
          </Text>
          <Text style={styles.body}>
        ...
        <Section last>
          <Text style={styles.version}>Version {Brand.version}</Text>
        </Section>
      </ScrollView>
    </View>
  );
